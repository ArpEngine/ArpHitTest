pipeline {
    ...
    environment {
        HAXELIB_CACHE = '../../.haxelib_cache/ArpHitTest/.haxelib'
    }
    ...
    stages {
        ...
        stage('swf') {
            steps {
                catchError {
                    sh "ARPCI_PROJECT=ArpHitTest ARPCI_TARGET=swf haxelib run arp_ci test"
                }
            }
            post {
        ...
        stage('js') {
            steps {
                catchError {
                    sh "ARPCI_PROJECT=ArpHitTest ARPCI_TARGET=js haxelib run arp_ci test"
                }
            }
            post {
